...
// Asset Studio: pick an asset type â†’ pick a template â†’ generate a draft
// artifact and open it in the editor. Slot-driven templates (Template Library)
// fill their slots from the product's approved messaging document â€” no prompt
...
export function Studio() {
  ...
  // Deck generation success toast (deck-studio.md Â§5.3) â€” shown briefly before
  ...
  const generate = async () => {
    ...
    try {
      if (template.generation_ready) {
        ...
        // final messaging doc. 409 = no final doc yet (Â§3.1 gate).
        ...
        setResult(r); // warnings or guard violations â€” surface before opening the editor
        return;
      }
      ...
      if (!r.degraded && r.guard.ok) {
        if (template.asset_type === "deck") {
          // Deck prompt path now lands structured slides (Â§4.5) â€” say so, then open.
          setToast("Deck generated â€” opening slide editor");
      ...
      setResult(r); // degraded or guard violations â€” surface before opening the editor
    } catch (e) {
  ...
  // Non-admins land on the canonical finalized surface in the workspace â€”
  // generation is PMM-only (blueprint workspace-tabs Â§5).
  ...
  return (
    <div>
      ...
      <p className="pagesub">
        Pick a type, a brand template, and a prompt â€” the draft lands in the library editor.
      </p>
          ...
          <div>
            <div className="card">
              ...
              <div className="empty-note" style={{ paddingBottom: 0 }}>
                ...
                messaging document. Others are mock previews served by the prompt path â€” browse and
            ...
            <div className="card">
              ...
              {slotDriven && (
                <div className="empty-note" style={{ paddingTop: 0 }}>
                  Not needed for a slot-driven template â€” the text comes from the approved messaging
              ...
              {prompts.map((p) => (
                ...
                >
                  ...
                  <div className="d">{p.body.length > 220 ? `${p.body.slice(0, 220)}â€¦` : p.body}</div>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="card">
            ...
            {slotDriven && productId === "" && (
              <p style={{ margin: "4px 0 0", fontSize: 11.5, color: "var(--text-secondary)" }}>
                Slot-driven generation runs from one product&rsquo;s approved messaging document â€”
                pick a product.
              </p>
            )}
            ...
            <input
              ...
              placeholder="e.g. Masterworks Build Datasheet â€” State DOTs"
            />
            ...
            <textarea
              ...
              placeholder="Persona, deal context, angles to emphasizeâ€¦"
            />
            ...
            {result && (
              <div style={{ marginTop: 16, borderTop: "1px solid var(--border)", paddingTop: 14 }}>
                ...
                {result.warnings !== undefined && result.warnings.length > 0 && (
                  <>
                    <div className="check-row">
                      ...
                      {result.warnings.length === 1 ? "s" : ""} PMM input â€” the render carries visible
                ...
                {result.degraded && (
                  <div className="check-row">
                    <i className="fa-solid fa-circle-xmark fail" /> AI generation unavailable â€” a starter scaffold
                    was created instead.
                  </div>
                )}
                {result.guard.ok ? (
                  <div className="check-row">
                    <i className="fa-solid fa-circle-check pass" /> Forbidden-words list â€” clear
                  </div>
                ) : (
                  <>
                    <div className="check-row">
                      <i className="fa-solid fa-circle-xmark fail" /> Forbidden-words list â€”{" "}
                    ...
                    >
                      {result.guard.violations.join(", ")} â€” fix these in the editor before the asset can move past
                      draft.
                    </div>
                  </>
                )}
                <div className="check-row">
                  {result.degraded ? (
                    <>
                      <i className="fa-solid fa-circle-xmark fail" /> Brand / voice rules â€” draft the sections in
                      the editor
                    </>
                  ) : (
                    <>
                      <i className="fa-solid fa-circle-check pass" /> Brand / voice rules â€” clear
                    </>
                  )}
                </div>
                <button
